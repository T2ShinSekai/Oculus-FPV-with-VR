...
public:
	...
	void UpdateVideoCapture(cv::Mat* mat);
...
class Node : public RefCountBase<Node>
{
    ...
    mutable bool      MatCurrent;	// false:0 true:gt>=1
...
public:
	...
	void ClearVCap(Texture* Tex, ID3D11InputLayout * VideoCapVertexIL);
...
class VideoCaptureDevice : public RefCountBase < VideoCaptureDevice >
{
	enum eyeno
	{
		Fst = 0,
...
public:
	cv::VideoCapture	Capture[count];
	cv::Mat				CvMat[count];
	cv::Mat				InputMat[count];

	VideoCaptureDevice()  { EyeSwitch = false; }
	~VideoCaptureDevice() { }
	...
	void initialize(int w, int h)
	{
		...
		{
			Capture[i].open(i);
			Capture[i].set(CV_CAP_PROP_FRAME_WIDTH, w);
			Capture[i].set(CV_CAP_PROP_FRAME_HEIGHT, h);
		}
		*/
	}
	void videoCapture()
	{
		...
		for (int i = 0; i < count; i++)
		{
			if (Capture[i].read(CvMat[i]))
				cv::cvtColor(CvMat[i], InputMat[i], CV_RGB2BGRA);
		}
		*/
	}
	cv::Mat GetInputMat(int eye)
	{
		int eyeno = 0;
		if (EyeSwitch) 
		{
...
class SerialPortController : public RefCountBase < SerialPortController >
{

public:
typedef struct COM_PORT_DESC_
{
	int			BaudRate;
	TCHAR	 	PortName[64];
	BYTE		Parity;
...
public:
	bool	OpenPort(COM_PORT_DESC* desc);
	bool	Send(LPCVOID outputData, const unsigned int& sizeBuffer, unsigned long& length);
	bool	Receive(char* inputData, const unsigned int& sizeBuffer, unsigned long& length);
	bool	ClosePort();
	bool	SendPacket(PACKET_INFO* Packet);
	void	SelectComPort();
	LPCWSTR	getPort(){ return lpPort_; };

};


class XbeeController : public RefCountBase < XbeeController >
{

public:
	typedef struct XBEE_DESC_
	{
		int			BaudRate;
		char	 	PortName[64];
		char		XbeeName[64];
...
public:
	bool	OpenPort(COM_PORT_DESC* desc);
	bool	Send(unsigned char* outputData, int length);
	bool	Receive(unsigned char* inputData, const unsigned int& sizeBuffer);
	bool	ClosePort();
	bool	SendPacket(PACKET_INFO* Packet);
	void	SelectComPort();
	char*	getPort() 
	{ 
		...
		WideCharToMultiByte(CP_ACP, 0, lpPort_, -1, str, sizeof(str), NULL, NULL);
		return str;
	}
