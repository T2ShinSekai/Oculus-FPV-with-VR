...
void Texture::UpdateVideoCapture(cv::Mat* mat)
{
	ID3D11Texture2D*			tex;
	ID3D11ShaderResourceView*   srv;

	D3D11_TEXTURE2D_DESC desc = { 0 };
	desc.Width		= mat->size().width;
	desc.Height		= mat->size().height;
	...
	D3D11_SUBRESOURCE_DATA d2InitData;
	d2InitData.pSysMem		= mat->data;
	d2InitData.SysMemPitch	= (UINT)mat->step[0];
	Ren->Device->CreateTexture2D(&desc, &d2InitData, &tex);
	...
	if (tex == NULL)
	{
		...
		// check "winerror.h" if it fails.
		D3DX11CreateTextureFromFile(Ren->Device, L"error.png", &info, NULL, &pTexture, &hr);
		tex = (ID3D11Texture2D*)pTexture;
	}
	if (tex != NULL)
	{ 
		Ren->Device->CreateShaderResourceView(tex, NULL, &srv);
		UpdatePlaceholderTexture(tex, srv);
	}
}


void Model::Render(const Matrix4f& ltw, RenderDevice* ren)	// called by Container::Render
{
    if (Visible)
    {
...
void Container::Render(const Matrix4f& ltw, RenderDevice* ren)	// called by Scene::Render
{
    Matrix4f m = ltw * GetMatrix();
...
void RenderDevice::ClearVCap(Texture* Tex, ID3D11InputLayout * VideoCapVertexIL)
{
	float color[] = { 1.0f, 0.25f, 0.45f, 1.0f };
	Context->IASetInputLayout(VideoCapVertexIL);
	//Context->ClearRenderTargetView(BackBufferRT, color);
	Context->ClearDepthStencilView(Tex->TexDsv, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);
}
...
void RenderDevice::Render(const Matrix4f& view, Model* model)	// called by Model::Render
{
                          ...
                          const Matrix4f& matrix, int offset, int count, PrimitiveType rprim, bool updateUniformData)
{
    ...
    unsigned char* vertexData = vshader->UniformData;	//TBD:::whts is inside?
    if (vertexData)
    {
		...
		vshader->SetUniformBuffer(UniformBuffers[Shader_Vertex]);	// Let it Execute "Ren->Context->VSSetConstantBuffers(i, 1, &((Buffer*)buffer)->D3DBuffer.GetRawRef());"
    }

    for(int i = Shader_Vertex + 1; i < Shader_Count; i++)
        if (shaders->GetShader(i))
        {
            ((ShaderBase*)shaders->GetShader(i))->UpdateBuffer(UniformBuffers[i]);		// Let it Execute "buf->Data(Buffer_Uniform, UniformData, UniformsSize);"
            ((ShaderBase*)shaders->GetShader(i))->SetUniformBuffer(UniformBuffers[i]);	// Let it Execute "Ren->Context->PSSetConstantBuffers(i, 1, &((Buffer*)buffer)->D3DBuffer.GetRawRef());"
        }
...
bool SerialPortController::OpenPort(COM_PORT_DESC* desc)
{
	if (isOpen_ == true)
		return TRUE;
	...
	{
		handlePort_ = CreateFile(desc->PortName,
		...
		config_.BaudRate = desc->BaudRate;    // Specify buad rate of communicaiton.
		config_.StopBits = desc->StopBits;    // Specify stopbit of communication.
		config_.Parity	 = desc->Parity;      // Specify parity of communication.
		config_.ByteSize = desc->ByteSize;    // Specify  byte of size of communication.
...
bool SerialPortController::ClosePort()
{
	if (isOpen_ == true)
		return TRUE;
...
bool SerialPortController::Send(LPCVOID outputData, const unsigned int& sizeBuffer, unsigned long& length)
{
	if (isOpen_ == true)
		return TRUE;
...
bool SerialPortController::Receive(char* inputData, const unsigned int& sizeBuffer, unsigned long& length)
{
	if (isOpen_ == true)
		return TRUE;
...
bool SerialPortController::SendPacket(PACKET_INFO* Packet)
{
	if (isOpen_ == true)
		return TRUE;

	unsigned long len;
	if (!Send(Packet, sizeof(PACKET_INFO), len)){ 
		isOpen_ = false;
		return FALSE;
	}
	return TRUE;
...
void SerialPortController::SelectComPort()
{
	...
	for (int i = 0; i<255; i++)
	{
		...
		check = QueryDosDevice(ComName, (LPWSTR)lpTargetPath, 1024);
		if (check != 0)
		{
			lpPort_ = ComName.AllocSysString();
			isOpen_ = false;
			break;
		}
...
void XbeeController::SelectComPort()
{
	TCHAR	lpTargetPath[1024];
	DWORD	check;
...
bool XbeeController::OpenPort(COM_PORT_DESC* desc)
{
	if (isOpen_ == true)
		return TRUE;
	...
	if (statusPort_ == false)  // if port is opened already, not open port again.
	{
		if ((ret = xbee_setup(&xbee, desc->XbeeName, desc->PortName, desc->BaudRate)) != XBEE_ENONE) {
		...
		address.addr64[5] = 0xAC;
		address.addr64[6] = 0x1B;
...
bool XbeeController::Send(unsigned char* outputData, int length)
{
	if (isOpen_ == true)
		return TRUE;
	...
	if (length > 0)
	{
		...
		xbee_connTx(con, &retVal, outputData, length);
		return TRUE;
	}
	return FALSE;
}

bool XbeeController::SendPacket(PACKET_INFO* Packet)
{
	if (isOpen_ == true)
		return TRUE;

	unsigned long len;
	if (!Send((unsigned char *)Packet, sizeof(PACKET_INFO))){
		isOpen_ = false;
		return FALSE;
	}
	return TRUE;
