...
#if SERVO_CTL
extern XbeeController SerialPort;
#endif
...
bool Util_RespondToControls(float & EyeYaw, Vector3f & EyePos, Quatf PoseOrientation)
{
	...
	#if SERVO_CTL
	XbeeController::PACKET_INFO Packet;
	Packet.Pan = calcRadianToAngleH(HeadYaw);
	Packet.Tilt = calcRadianToAngleV(tempHeadPitch);
	SerialPort.SendPacket(&Packet);
	#endif
